...
def test_login_nomal(envi, url=url, loginname='admin', password='ghcatest'):
    logger_debug('测试正常登录')
...
def test_logining_four_false(envi, url=url, loginname='ghca', password='11'):
    logger_debug("测试连续登录四次登录失败")
    four_result=login_times(url=url,loginName=loginname,password=password, times=4)
    ...
    envi.redis.detete_keys("login:fail:*")

    result_query_none = access_audit_query(envi.admin)
    assert result_query_none.response["data"] == []

    result_five = login_times(url=url, loginName=loginname, password="111111", times=1)
...
def test_loging_five_flse_and_ip_forbidden(koal, url=url):
    ...
    """
    login_false_list = login_times(url=url, loginName='ghca', password='111', times=3)
    assert login_false_list[2].success is False

    result_login_false_list = login_times(url, loginName='liulj', password='111', times=2)
    ...
    result_login_nomal = login_times(url,loginName='ghca',password='111111',times=1)
    assert result_login_nomal[0].success is True

    result_login_nomal2 = login_times(url,loginName='liulj',password='111111',times=1)
    assert result_login_nomal2[0].success is True, result_login_nomal2[0].error
if __name__ == "__main__":
    pytest.main(["-s", "test_00_login.py::test_logining_four_false",'--alluredir', './temp'])
