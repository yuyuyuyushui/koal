...
def test_edit_organize_nomal(koal,parentid=0,dept_old_name='test_edit_{}'.format(randint(1,999)),dept_new_Name='test_edit_new_{}'.format(randint(1,999)),):
    ...
    logger_info('测试编辑部门')
    response_add_organize = add_organize(koal,parentid,dept_old_name)
    assert response_add_organize.success==True, response_add_organize.error
    dept_response = first_level_dept_name_and_get_deptId(koal, dept_old_name)
...
if __name__=='__main__':
    pytest.main(['-s', "test_02_organize_edit.py::test_edit_organize_nomal"])