...
def rehash(path, algo='sha256', blocksize=1 << 20):
    """Return (hash, length) for filepath using hashlib.new(algo)"""
    h = hashlib.new(algo)
    length = 0
...
def fix_script(path):
    """Replace #!python with #!/filepath/to/python
...
def message_about_scripts_not_on_PATH(scripts):
    ...
    # Group scripts by the filepath they were installed in
...
def move_wheel_files(name, req, wheeldir, user=False, home=None, root=None,
    ...
    def clobber(source, dest, is_base, fixer=None, filter=None):
        ensure_dir(dest)  # common for the 'include' filepath
